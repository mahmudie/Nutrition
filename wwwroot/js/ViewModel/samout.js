function SamoutVm(model) {
    ...
    self.partialForm = {
        OalsKwashiorkor: ko.observable(model.oalsKwashiorkor),
        OalsMarasmus: ko.observable(model.oalsMarasmus),
        OawgKwashiorkor: ko.observable(model.oawgKwashiorkor),
        OawgMarasmus: ko.observable(model.oawgMarasmus),
    }

        self.questions = {
        Nmrid:model.nmrid,
        Bnaqid:model.bnaqid,
        OpdRutfstockOutWeeks: ko.observable(model.opdRutfstockOutWeeks).extend({ numeric: 0 }),
        OpdAdmissionsByChws: ko.observable(model.opdAdmissionsByChws).extend({ numeric: 0 }),
    }
    ...
    self.partialRecord = function (form) {
        ...
        self.questions.__RequestVerificationToken = token;
             $.ajax({
                     url: '/Bnaq/edit/'+self.questions.Nmrid+'/'+2,
                     type: 'POST',
                 ...
                 .success(function(){
                    ...
                    ajaxHelper("/api/samout/partial/" + [self.reports.Nmrid], 'PUT', ko.toJS(self.partialForm), token).success(function (data) {
                        $("#send").notify("Successfully Saved.", "success");
                    }).fail(function () {
    ...
    function calctotal(item) {
        var token = $("input[name=__RequestVerificationToken]").val();
        ajaxHelper(uri + "/total/" + item.otpid + "nmrid=" + item.nmrid, 'get', null, token)
            .done(function (data) {
                if (data > 0) {
    ...
    self.newReportC = function (form) {
        if (!$(form).valid())
            return false;
        self.sending(true);
        var token = form["__RequestVerificationToken"].value;
        ajaxHelper(uri, 'POST', ko.toJS(self.reports), token)
            .done(function (item) {
                self.rep.push(item);
                self.sending(false);
                $('#new').modal('hide');
            })
            .complete(self.sending(false));
    }


    self.delete = function (item) {
        var token = $("input[name=__RequestVerificationToken]").val();
        ajaxHelper(uri + '/' + item.otpid + 'id2=' + item.nmrid, 'DELETE', null, token)
    ...
    self.checkInputs = ko.computed(function () {
        ...
        var balance = self.reports.Totalbegin() ? parseInt(self.reports.Totalbegin()) : 0;
            return (cured + noncured + death + dflt + refout <= balance + female + male + defaultreturn + fromsfp + fromscotp);
    ...
    self.edit = function (item) {
        ...
        map(item);
        calctotal(item);
        $.get("/samout/edit/" + item.otpid + '?nmrid=' + item.nmrid, function (d) {
            $('#popup').html(d);
            $('#edit').modal('show');
    ...
    self.editRecord = function (form) {
        ...
        self.sending(true);
        var token = form["__RequestVerificationToken"].value;
        ajaxHelper(uri + "/" + self.reports.Otpid + "nmrid=" + [self.reports.Nmrid], 'PUT', ko.toJS(self.reports), token).success(function (data) {
            getReports();
            $('#edit').modal('hide');
    ...
    self.editStock = function (item) {
        ...
        mapStock(item);
        $.get("/Samout/EditStock/" + item.stockId + "?nmrid=" + item.nmrid, function (d) {
            $('#popup').html(d);
    ...
    self.editstockRecord = function (form) {
        ...
        self.sending(true);
        var token = form["__RequestVerificationToken"].value;
        ajaxHelper(uri + "/stock/" + self.sinStock.SstockotpId + "nmrid=" + [self.sinStock.Nmrid], 'PUT', ko.toJS(self.sinStock), token).success(function (data) {
            getStocks();
            $.notify("Successfully Saved.", "success");
        ...
        return {
            sum: [sum1, sum2, sum3, sum4, sum5, sum6, sum5 + sum6, sum7, sum8, sum9, sum7 + sum8 + sum9,
                ...
                ending, Math.floor((sum10 / exit) * 100) ? Math.floor((sum10 / exit) * 100) + ' %' : 0, Math.floor((sum11 / exit) * 100) ? Math.floor((sum11 / exit) * 100) + ' %' : 0,
                 Math.floor((sum12 / exit) * 100) ? Math.floor((sum12 / exit) * 100) + ' %' : 0, Math.floor((sum13 / exit) * 100) ? Math.floor((sum13 / exit) * 100) + ' %' : 0, Math.floor((sum14 / exit) * 100) ? Math.floor((sum14 / exit) * 100) + '%' : 0
            ]
        };
    });
    ...
    function map(data) {
        ...
        self.reports.Otpid = data.otpid;
        self.reports.UserName = data.userName;
    }

    function mapStock(data) {
        self.sinStock.SstockotpId = data.stockId;

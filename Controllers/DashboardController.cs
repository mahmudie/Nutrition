...
namespace DataSystem.Controllers
{
    ...
    public class DashboardController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            items.OrderByDescending(m => m.Year);
            items.Insert(0, new { Year = 0, Des = "All" });
            ViewData["years"] = new SelectList(items, "Year", "Des").OrderBy(m=>m.Value);
            return View();
        }


        [HttpGet("/dashboard/dashStat/{year}")]
        public IActionResult dashStat(int year)
        {
            ...
            if (year != 0)
            {
                get = _db.TblOtp.Where(m => m.Nmr.Year == year);
                getm = _db.TblMam.Where(m => m.Nmr.Year == year);
                get2 = _db.TblOtptfu.Where(m => m.Nmr.Year == year);
            }
        ...
        [HttpGet("/dashboard/provAll/{year}")]
        public IActionResult provAll(int year)
        {
            ...
            if (year != 0)
            {
                get = _db.TblOtp.Where(m => m.Nmr.Year == year && !m.Otp.AgeGroup.ToLower().Contains("total"));
                getm = _db.TblMam.Where(m => m.Nmr.Year == year && !m.Mam.AgeGroup.ToLower().Contains("total"));
                get2 = _db.TblOtptfu.Where(m => m.Nmr.Year == year && !m.Otptfu.AgeGroup.ToLower().Contains("total"));
            }
            ...
            foreach (var item in data.Select(m => m.PROV_32_ID).Distinct())
            {
                ...
                //row.NonCured = data.Where(m => m.PROV_32_ID.Equals(item)).Sum(m => m.NonCured);
                //row.NonCuredP = data.Where(m => m.PROV_32_ID.Equals(item)).Sum(m => m.NonCured);
